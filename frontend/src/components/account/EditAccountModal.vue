<template>
  ...
  >
    ...
    >
      ...
      <!-- Codex quota overdraft: one user-facing switch for both quota windows. -->
      <div
      ...
      >
        <div class="flex items-center justify-between gap-4">
          ...
          <button
            ...
            @click="toggleCodexQuotaOverdraft"
            :class="[
              'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2',
      ...
      >
        ...
        <div class="rounded-lg border border-gray-200 p-4 dark:border-dark-600">
          ...
          <div v-if="tlsFingerprintEnabled" class="mt-3">
            <select v-model.number="tlsFingerprintProfileId" class="input" data-testid="edit-tls-fingerprint-profile">
      ...
      <!-- TLS fingerprint for OpenAI OAuth accounts -->
      <div
        v-if="account?.platform === 'openai' && account?.type === 'oauth'"
        class="border-t border-gray-200 pt-4 dark:border-dark-600"
      >
        <div class="rounded-lg border border-gray-200 p-4 dark:border-dark-600">
          ...
          <div v-if="tlsFingerprintEnabled" class="mt-3">
            <select v-model.number="tlsFingerprintProfileId" class="input" data-testid="edit-openai-tls-fingerprint-profile">
              <option :value="null">{{ t('admin.accounts.quotaControl.tlsFingerprint.defaultProfile') }}</option>
              <option v-if="tlsFingerprintProfiles.length > 0" :value="-1">{{ t('admin.accounts.quotaControl.tlsFingerprint.randomProfile') }}</option>
              <option v-for="p in tlsFingerprintProfiles" :key="p.id" :value="p.id">{{ p.name }}</option>
            </select>
...
const codexQuotaOverdraftEnabled = computed(
  () => autoPause5hDisabled.value && autoPause7dDisabled.value
)
const toggleCodexQuotaOverdraft = () => {
  const enabled = !codexQuotaOverdraftEnabled.value
  autoPause5hDisabled.value = enabled
  autoPause7dDisabled.value = enabled
}
...
// Load quota controls and shared TLS fingerprint settings from account.
function loadQuotaControlSettings(account: Account) {
  ...
  // New API responses expose flattened fields. Fall back to extra for older
  // responses and list snapshots so persisted OpenAI OAuth settings still
  // rehydrate correctly during rolling upgrades.
  tlsFingerprintEnabled.value =
    typeof account.enable_tls_fingerprint === 'boolean'
      ? account.enable_tls_fingerprint
      : extra?.enable_tls_fingerprint === true
  tlsFingerprintProfileId.value = toPositiveNumber(
    account.tls_fingerprint_profile_id ?? extra?.tls_fingerprint_profile_id
  )
    ...
    if (props.account.platform === 'openai' && (props.account.type === 'oauth' || props.account.type === 'setup-token' || props.account.type === 'apikey')) {
      ...
      if (props.account.type === 'oauth') {
        if (tlsFingerprintEnabled.value) {
          ...
          // Persist an explicit opt-out so a later default migration cannot
          // silently re-enable the setting.
