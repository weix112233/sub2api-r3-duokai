    ...
    >
      ...
      >
        ...
        <div class="rounded-lg border border-gray-200 p-4 dark:border-dark-600">
          ...
          <div v-if="tlsFingerprintEnabled" class="mt-3">
            <select v-model.number="tlsFingerprintProfileId" class="input" data-testid="create-tls-fingerprint-profile">
      ...
      >
        <div class="rounded-lg border border-gray-200 p-4 dark:border-dark-600">
          ...
          <div v-if="tlsFingerprintEnabled" class="mt-3">
            <select v-model.number="tlsFingerprintProfileId" class="input" data-testid="create-openai-tls-fingerprint-profile">
              <option :value="null">{{ t('admin.accounts.quotaControl.tlsFingerprint.defaultProfile') }}</option>
              <option v-if="tlsFingerprintProfiles.length > 0" :value="-1">{{ t('admin.accounts.quotaControl.tlsFingerprint.randomProfile') }}</option>
              <option v-for="p in tlsFingerprintProfiles" :key="p.id" :value="p.id">{{ p.name }}</option>
            </select>
...
const buildOpenAIExtra = (base?: Record<string, unknown>): Record<string, unknown> | undefined => {
  ...
  // Persist the default explicitly for OpenAI OAuth imports; other OpenAI
  // account types do not use the Codex fingerprint policy.
  if (form.type === 'oauth') {
