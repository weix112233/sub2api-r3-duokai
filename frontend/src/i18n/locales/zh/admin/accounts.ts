    ...
    accounts: {
      ...
      openai: {
        ...
        codexQuotaOverdraftDesc:
          '仅对 OpenAI OAuth 生效。开启后，额度探测确认上游仍可用时，本账号可继续使用 5 小时和 7 天额度；关闭后完全采用官方调度和额度处理。',
        ...
        codexFingerprintModeDesc: '多人共享同一 OAuth 账号时，将各用户的设备/会话标识收敛为账号级恒定值，减少上游可见的设备数和会话数。本基线默认「单机多窗口」，关闭则原样透传客户端标识。「单机多窗口」把设备收敛为一台，各下游会话按账号密钥 1:1 假名化（保留会话边界与缓存键稳定），对 Codex 客户端删除其已不再发送的下划线会话头，上游看到一台设备开多个 Codex 窗口；非 Codex 客户端只假名化缓存键、不合成 Codex 窗口身份。',
