    ...
    settings: {
      ...
      features: {
        ...
        riskControl: {
          ...
          antiBypass: '网关防破限',
          antiBypassHint: '开启后按认证用户统一统计跨 Key、IP 和客户端指纹，并在转发前拦截重放与高置信提示词越狱。默认关闭。',
      ...
      gatewayForwarding: {
        ...
        codexAllowAppServerDesc:
          '放行内嵌 Codex 引擎、经 app-server 协议接入的第三方客户端（如 Claude Code 的 codex 插件）。默认开启；关闭后此类客户端通过引擎指纹门（下方信号列表）才可放行，未关闭时仍仅放行官方客户端与白名单。',
