    ...
    settings: {
      ...
      features: {
        ...
        riskControl: {
          ...
          antiBypass: 'Gateway anti-bypass protection',
          antiBypassHint: 'When enabled, requests are unified by authenticated user across keys, IPs, and client fingerprints; replay and high-confidence prompt jailbreaks are blocked before forwarding. Disabled by default.',
      ...
      gatewayForwarding: {
        ...
        codexAllowAppServerDesc:
          "Allow third-party clients that embed the Codex engine and connect over the app-server protocol (e.g. Claude Code's codex plugin). On by default; when off, such clients are only allowed once they pass the engine-fingerprint gate (the signal list below); otherwise only official clients and the whitelist are allowed.",
