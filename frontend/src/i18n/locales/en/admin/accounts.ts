    ...
    accounts: {
      ...
      openai: {
        ...
        codexQuotaOverdraftDesc:
          'OpenAI OAuth only. When enabled, this account may continue using its 5-hour and 7-day quota while a quota probe still confirms upstream availability; when disabled, official scheduling and quota handling apply.',
        ...
        codexFingerprintModeDesc: 'When multiple users share the same OAuth account, converge device/session identifiers to account-level stable values to reduce upstream-visible device and session count. This baseline defaults to Single machine, multi-window; Off passes client identifiers through as-is. "Single machine, multi-window" converges the device to one, pseudonymizes each downstream session 1:1 with the account key (session boundaries and cache keys stay stable), and for Codex clients drops the underscore session headers real Codex no longer sends, so upstream sees one device running several Codex windows; non-Codex clients only get their cache key pseudonymized, no Codex window identity is synthesized.',
