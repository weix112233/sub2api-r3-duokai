> 最新ソースは **R10** です。`6288461` のモデル容量エラー分類修正を含みます。
> 現在のバージョンと配布状況は [RELEASE-R10.md](RELEASE-R10.md) を参照してください。
> R7 のタグとパッケージは過去のロールバック用であり、最新版ではありません。
> ソースの公開は、稼働中のサービスの更新完了を意味しません。
