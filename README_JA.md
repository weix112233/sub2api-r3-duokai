> 最新ソースは **R11** です。HTTP 追跡 ID の互換性修正と OAuth セッション維持の変更を含みます。
> 検証と配布状況は [RELEASE-R11.md](RELEASE-R11.md) を参照してください。R11 のバイナリ検証と公開はまだ完了していません。
> R10（`10e35ba`）は過去のソースです。R11 は現在の `main` を使用してください。古い R10 アーカイブには本修正が含まれません。
> ソースのコミットは、稼働中のサービスの更新完了を意味しません。
> 今後の正式配布には macOS、Windows、Linux のパッケージが必要です。[RELEASE-POLICY.md](RELEASE-POLICY.md) を参照してください。
