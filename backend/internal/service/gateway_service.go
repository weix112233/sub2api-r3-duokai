...
const GatewayFailureReasonProxyChainUnavailable GatewayFailureReason = "proxy_chain_unavailable"
...
func (e *UpstreamFailoverError) IsProxyChainFailure() bool {
	return e != nil && e.Reason == GatewayFailureReasonProxyChainUnavailable
}
...
// 在同账号重试全部用尽、切换账号时执行账号终态处置。
func (s *GatewayService) TempUnscheduleRetryableError(ctx context.Context, accountID int64, failoverErr *UpstreamFailoverError) {
	applyRetryableErrorTerminalDisposition(ctx, s.accountRepo, s.rateLimitService, accountID, failoverErr)
}

// applyRetryableErrorTerminalDisposition is shared by the legacy OpenAI
// handlers and the newer GatewayService failover loop so retry exhaustion
// cannot bypass account-state cleanup.
func applyRetryableErrorTerminalDisposition(
	ctx context.Context,
	...
	case http.StatusUnauthorized:
		// 池模式同账号重试耗尽后，401 已不再是“可通过换同一账号内部成员”
		// 解决的瞬时错误；继续让该账号回池会把坏凭据放大到后续请求。
		// 只写通用状态，不读取或记录凭据/正文。
		account := accountForRetryTerminalDisposition(ctx, accountRepo, accountID)
		msg := "Authentication failed (401): pool-mode retry budget exhausted"
		if rateLimitService != nil {
			rateLimitService.handleAuthError(ctx, account, msg)
		} else if err := accountRepo.SetError(ctx, accountID, msg); err != nil {
			slog.Warn("pool_mode_401_set_error_failed", "account_id", accountID, "error", err)
		}
	case http.StatusTooManyRequests:
		// 429 无 reset 时使用可配置的长兜底冷却，避免同一坏账号在每个请求中
		// 重新进入调度。真实 reset 时间仍由 HandleUpstreamError 优先使用。
		account := accountForRetryTerminalDisposition(ctx, accountRepo, accountID)
		if rateLimitService != nil {
			rateLimitService.handlePoolModeRetryExhausted429(ctx, account, failoverErr.ResponseHeaders, failoverErr.ResponseBody)
		} else {
			resetAt := time.Now().Add(5 * time.Minute)
			if err := accountRepo.SetRateLimited(ctx, accountID, resetAt); err != nil {
				slog.Warn("pool_mode_429_set_rate_limit_failed", "account_id", accountID, "error", err)
			}
		}
	case http.StatusBadRequest:
		tempUnscheduleGoogleConfigError(ctx, accountRepo, accountID, "[handler]")
	case http.StatusBadGateway:
		tempUnscheduleEmptyResponse(ctx, accountRepo, accountID, "[handler]")
	}
}

func accountForRetryTerminalDisposition(ctx context.Context, accountRepo AccountRepository, accountID int64) *Account {
	if accountRepo != nil {
		if account, err := accountRepo.GetByID(ctx, accountID); err == nil && account != nil {
			return account
		}
	}
	return &Account{ID: accountID, Platform: PlatformOpenAI, Type: AccountTypeAPIKey}
}
