...
func (s *PricingService) checkAndUpdatePricing() error {
	...
	// A local pricing file is sufficient when remote synchronization is
	// intentionally disabled. Do not age-trigger a download with an empty URL.
	if strings.TrimSpace(s.cfg.Pricing.RemoteURL) == "" {
		return nil
	}
