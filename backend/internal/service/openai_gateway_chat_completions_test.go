...
func TestForwardAsChatCompletions_APIKeyPropagatesPromptCacheKeyInResponsesBody(t *testing.T) {
	...
	require.Error(t, err)
	require.Nil(t, result)
	rewrittenPromptCacheKey := gjson.GetBytes(upstream.lastBody, "prompt_cache_key").String()
	require.NotEmpty(t, rewrittenPromptCacheKey)
	require.NotEqual(t, "cache-key-123", rewrittenPromptCacheKey)
	require.Contains(t, rewrittenPromptCacheKey, openAIOutboundPromptCacheKeyPrefix)
