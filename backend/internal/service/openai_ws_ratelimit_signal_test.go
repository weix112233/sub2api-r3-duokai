...
func TestOpenAIWSRateLimitFailoverError_OAuthKeepsSameAccountDeadline(t *testing.T) {
	...
	longRetryErr := svc.newOpenAIWSRateLimitFailoverError(&Account{
		...
		Platform: PlatformOpenAI,
		Type:     AccountTypeOAuth,
	}, http.Header{"Retry-After": []string{"30"}}, body, "limited")
	require.False(t, longRetryErr.RetryableOnSameAccount)
	require.True(t, longRetryErr.SameAccountRetryDeadline.IsZero())
	require.Zero(t, longRetryErr.SameAccountRetryDelay)
