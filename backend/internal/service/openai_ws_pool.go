...
retryAcquire:
	...
	if allowReuse {
		...
		// 指纹收敛分桶移植：稳定身份（握手兼容键）匹配即可复用；路由提示只是
		// 软亲和——无提示或已达容量上限时不再按亲和过滤（否则兼容连接会被
		// 驱逐重拨，破坏"同一稳定身份复用同一连接"的上游会话语义）。
...
// pickOldestIdleConnWithoutHandshakeCompatibilityLocked 只把「握手不兼容」的空闲
// 连接列为缩容驱逐候选。兼容连接即使路由亲和不同也不驱逐（软亲和语义，
// 与上方复用循环的门控一致）。
