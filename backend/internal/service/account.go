...
// IsTLSFingerprintEligible reports whether this account type may use a
// configured TLS fingerprint profile.
func (a *Account) IsTLSFingerprintEligible() bool {
	if a == nil {
		return false
	}
...
// 支持 Anthropic OAuth/SetupToken 和 OpenAI OAuth；API key 账号不适用。
func (a *Account) IsTLSFingerprintEnabled() bool {
	if !a.IsTLSFingerprintEligible() {
		return false
	}
	if a.Extra == nil {
		return false
...
// NormalizeTLSFingerprintExtraForCreate makes TLS fingerprinting explicit for
// new eligible accounts. An explicit boolean supplied by the caller is always
// preserved so users can opt out.
func NormalizeTLSFingerprintExtraForCreate(account *Account) {
	if account == nil || !account.IsTLSFingerprintEligible() {
		return
	}
	if account.Extra == nil {
		account.Extra = make(map[string]any, 1)
	}
	if _, ok := account.Extra["enable_tls_fingerprint"].(bool); ok {
		return
	}
	account.Extra["enable_tls_fingerprint"] = true
}

// NormalizeAnthropicTLSFingerprintExtraForCreate remains as a compatibility
// wrapper for callers outside the current repository.
func NormalizeAnthropicTLSFingerprintExtraForCreate(account *Account) {
