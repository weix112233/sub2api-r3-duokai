...
func (s *OpenAIGatewayService) forwardOpenAIPassthrough(
	...
	if account != nil && account.Type == AccountTypeOAuth {
		...
		// machine 模式下 compact 也 resolve/stage（与非透传路径同门控，见 openai_gateway_forward.go）。
		if !isOpenAIResponsesCompactPath(c) || account.GetCodexFingerprintMode() == codexFingerprintMachine {
			var clientHeaders http.Header
			if c != nil && c.Request != nil {
				clientHeaders = c.Request.Header
			}
			fpIDs := resolveCodexFingerprintIDsFromRequest(account, clientHeaders)
			// machine：按本链路终态出站 UA（与 buildUpstreamRequestOpenAIPassthrough 的 enforceCodexIdentityHeadersWithUA 同源）补记 sandbox 标签。
...
func (s *OpenAIGatewayService) buildUpstreamRequestOpenAIPassthrough(
	...
	if clientPromptCacheKey == "" {
		clientPromptCacheKey = explicitOpenAIRequestSessionID(c, body)
	}
	...
	} else if account != nil && account.Platform == PlatformOpenAI && !isOpenAIResponsesCompactPath(c) {
	...
	if shouldUseOpenAIGatewaySessionAffinity(account) {
		// r3 透传源头语义：session/conversation 各自按客户端显式下划线头
		// （缺失时回退 cache key）独立隔离；仅发送本地隔离值，原始标识由
		// 终态 sanitizer 删除。conversation 的自动兜底仅限非 messages 桥。
		clientSessionSeed := ""
		clientConversationSeed := ""
		if c != nil && c.Request != nil {
			clientSessionSeed = strings.TrimSpace(c.Request.Header.Get("session_id"))
			clientConversationSeed = strings.TrimSpace(c.Request.Header.Get("conversation_id"))
		}
		if machineStaged {
			// machine：非透传锚出站 pck'（假名，本链路 clientPromptCacheKey 读自
			// sink 改写后的 body 即 pck' 本身）；客户端带下划线 session_id（第三方
			// Agent 形态）时优先锚定该本地会话标识。messages 桥不带 body pck 时
			// 回退客户端 cache key。conversation 不做 machine 假名锚——客户端没发
			// 就不补（machine 只改不增）。
			...
			if clientSessionSeed != "" {
				affinitySeed = clientSessionSeed
			}
			...
			sessionSeed := clientSessionSeed
			if sessionSeed == "" {
				sessionSeed = strings.TrimSpace(clientPromptCacheKey)
			}
			if sessionSeed == "" && isOpenAIResponsesCompactPath(c) {
				// r3 语义：compact 路径无 cache key 时以 compact 会话解析结果为锚。
				sessionSeed = strings.TrimSpace(resolveOpenAICompactSessionID(c))
			}
			conversationSeed := clientConversationSeed
			if conversationSeed == "" && !compatMessagesBridge {
				conversationSeed = strings.TrimSpace(clientPromptCacheKey)
			}
			sessionAffinity = deriveOpenAIGatewaySessionAffinityWithIndependentSeeds(
				getAPIKeyIDFromContext(c),
				sessionSeed,
	...
	// 透传客户端请求头（安全白名单；Codex 会话身份头仅 machine 模式放行）。
	allowTimeoutHeaders := s.isOpenAIPassthroughTimeoutHeadersAllowed()
	codexMachine := activeCodexFingerprintMode(account) == codexFingerprintMachine
	if c != nil && c.Request != nil {
		for key, values := range c.Request.Header {
			lower := strings.ToLower(strings.TrimSpace(key))
			if !isOpenAIPassthroughAllowedRequestHeader(lower, allowTimeoutHeaders, codexMachine) {
				continue
			}
	...
	if account.Type == AccountTypeOAuth {
		...
		// Do not reintroduce client session/conversation identifiers after the
		// final header sanitizer. Session affinity headers are gateway-owned
		// values derived from the local conversation anchor (see
		// deriveOpenAIGatewaySessionAffinity above) and are applied just before
		// the fingerprint header sink.
	...
	// 会话亲和头（网关自有隔离值）先落，再交给指纹头 sink——machine 分支会按
	//「只改不增」删除 Codex 形态下游的下划线会话头（第三方形态保留）。
...
func shouldFailoverOpenAIPassthroughResponse(account *Account, statusCode int, responseBody []byte) bool {
	...
	// OpenAI may return a Responses `response.failed` capacity envelope with an
	// HTTP 5xx status instead of the usual HTTP 200 SSE sequence. This is a
	// request-scoped transient, not an account-specific OAuth/API-key failure.
	// Classify the body before account/type-specific status rules so OAuth
	// passthrough requests do not commit the error to the client prematurely.
...
// isOpenAIPassthroughAllowedRequestHeader 透传 /responses 链路的客户端头放行判定；
// codexMachine 为账号有效指纹模式是否为 machine（activeCodexFingerprintMode）。
func isOpenAIPassthroughAllowedRequestHeader(lowerKey string, allowTimeoutHeaders bool, codexMachine bool) bool {
	if lowerKey == "" {
		return false
	}
	...
	// Codex 会话身份头仅在 machine 模式的 /responses 透传链路放行（images 链路直接查 openaiPassthroughAllowedHeaders，不经此处）。
	return openaiPassthroughAllowedHeaders[lowerKey] || (codexMachine && openaiCodexSessionIdentityHeaders[lowerKey])
}
...
func openAIStreamEventShowsProgress(eventType string) bool {
	...
	case "error", "response.failed":
		return false
	default:
		// Any non-preamble provider event means the upstream has made semantic
		// progress. This is intentionally separate from client-output commitment:
		// empty structural events can stop the first-output timer while remaining
		// buffered and retry-safe.
		return true
	}
}

func openAIStreamDataShowsProgress(data, eventType string) bool {
	trimmed := strings.TrimSpace(data)
	if trimmed == "" || trimmed == "[DONE]" || !gjson.Valid(trimmed) {
		return false
	}
	return openAIStreamEventShowsProgress(eventType)
}
...
func openAIStreamDataStartsClientOutput(data, eventType string) bool {
	...
	// Some upstream overload frames carry an SSE error envelope without a
	// JSON `type` field. Keep retryable error envelopes buffered so the
	// following response.failed frame can still enter failover.
	normalizedEventType := strings.TrimSpace(eventType)
	if (normalizedEventType == "" || normalizedEventType == "error") &&
		openAIStreamErrorEnvelopePresent(trimmed) &&
		openAIStreamFailedEventShouldFailover([]byte(trimmed), extractOpenAISSEErrorMessage([]byte(trimmed))) {
		return false
	}
	if normalizedEventType == "" {
		normalizedEventType = strings.TrimSpace(gjson.Get(trimmed, "type").String())
	}
	...
	case "response.output_text.delta",
		...
		return openAIStreamValueHasSemanticContent(gjson.Get(trimmed, "delta"))
	case "response.output_text.done",
		"response.reasoning_summary_text.done",
		"response.reasoning_text.done",
		"response.audio_transcript.done":
		return openAIStreamValueHasSemanticContent(gjson.Get(trimmed, "text")) ||
			openAIStreamValueHasSemanticContent(gjson.Get(trimmed, "transcript"))
	case "response.function_call_arguments.done":
		return openAIStreamValueHasSemanticContent(gjson.Get(trimmed, "arguments"))
	case "response.custom_tool_call_input.done":
		return openAIStreamValueHasSemanticContent(gjson.Get(trimmed, "input"))
	case "response.image_generation_call.partial_image":
		return openAIStreamValueHasSemanticContent(gjson.Get(trimmed, "partial_image_b64"))
	case "response.content_part.added", "response.content_part.done",
		"response.reasoning_summary_part.added", "response.reasoning_summary_part.done":
		return openAIStreamObjectHasSemanticContent(gjson.Get(trimmed, "part"))
	case "response.output_item.added", "response.output_item.done":
		return openAIStreamObjectHasSemanticContent(gjson.Get(trimmed, "item"))
	case "response.completed", "response.done",
		"response.incomplete", "response.cancelled", "response.canceled":
		// Empty completed/done events are handled by the caller before writing.
		// Other terminal events are committed conservatively and are never
		// replayed across accounts after the stream has reached a terminal state.
		return true
	}
	// Provider-specific events may carry semantic output that this gateway does
	// not understand. Fail closed: treating them as committed is safer than
	// replaying a partially consumed request on another account.
	return true
}

func openAIStreamErrorEnvelopePresent(payload string) bool {
	return gjson.Get(payload, "error").Exists() ||
		gjson.Get(payload, "response.error").Exists()
}
...
func openAIStreamValueHasSemanticContent(value gjson.Result) bool {
	if !value.Exists() {
		return false
	}
	switch value.Type {
...
func openAIStreamObjectHasSemanticContent(object gjson.Result) bool {
	if !object.Exists() || !object.IsObject() {
		return false
	}
	if openAIStreamItemHasVisibleOutput(object) {
		return true
	}
	for _, path := range []string{
		...
		if openAIStreamValueHasSemanticContent(object.Get(path)) {
			return true
		}
	}
	return false
...
// （通常 code=server_is_overloaded / slow_down，也可能只带过载消息）
// 并以 response.failed 收尾。
func isOpenAIUpstreamCapacityShedEvent(payload []byte) bool {
	switch openAIStreamFailedEventErrorCode(payload) {
	case "server_is_overloaded", "slow_down":
		return true
	}
	message := strings.ToLower(strings.TrimSpace(extractOpenAISSEErrorMessage(payload)))
	if message == "" {
		return false
	}
...
func (s *OpenAIGatewayService) recordOpenAIStreamUpstreamError(
	...
	return s.recordOpenAIStreamUpstreamErrorWithUsage(
		c, account, passthrough, upstreamRequestID, kind, payload, message, nil,
	)
}

func (s *OpenAIGatewayService) recordOpenAIStreamUpstreamErrorWithUsage(
	c *gin.Context,
	account *Account,
	passthrough bool,
	upstreamRequestID string,
	...
	message string,
	usage *OpenAIUsage,
	streamCommit ...openAIStreamCommitState,
) string {
	...
	if isOpenAIUpstreamCapacityShedEvent(payload) {
		MarkOpenAIRequestScopedTransient(c)
	}
	...
	if c != nil {
		...
		if len(streamCommit) > 0 {
			state := streamCommit[0]
			event.ClientOutputStarted = state.ClientOutputStarted
			event.FirstClientOutputEventType = strings.TrimSpace(state.FirstClientOutputEventType)
			event.VisibleOutputSeen = state.VisibleOutputSeen
		}
		if usage != nil {
			usageCopy := *usage
...
type openAIStreamCommitState struct {
	ClientOutputStarted        bool
	FirstClientOutputEventType string
	VisibleOutputSeen          bool
}
...
func (s *OpenAIGatewayService) newOpenAIStreamFailoverError(
	...
	return s.newOpenAIStreamFailoverErrorWithUsage(
		c, account, passthrough, upstreamRequestID, payload, message, nil, responseHeaders...,
	)
}

func (s *OpenAIGatewayService) newOpenAIStreamFailoverErrorWithUsage(
	c *gin.Context,
	account *Account,
	passthrough bool,
	upstreamRequestID string,
	...
	message = s.recordOpenAIStreamUpstreamErrorWithUsage(
		c, account, passthrough, upstreamRequestID, "failover", payload, message, usage,
	)
...
func (s *OpenAIGatewayService) handleStreamingResponsePassthrough(
	...
	streamProgressObserved := false
	eventShowsProgress := false
	failedMessage := ""
	clientOutputStarted := false
	postOutputFailureRecorded := false
	...
	for documentScanner.Scan() {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			eventShowsProgress = eventShowsProgress || openAIStreamDataShowsProgress(trimmedData, eventType)
			if eventType == "response.failed" {
				...
				if !openAIStreamClientOutputStarted(c, clientOutputStarted) {
					// Capacity shed is a request-scoped transient. It must enter the
					// bounded same-account retry path before any configurable
					// passthrough rule can commit a terminal client error.
					if isOpenAIUpstreamCapacityShedEvent(dataBytes) {
						return resultWithUsage(),
							s.newOpenAIStreamFailoverErrorWithUsage(c, account, true, upstreamRequestID, dataBytes, failedMessage, usage, resp.Header)
					}
					if status, errType, errMsg, matched := applyOpenAIStreamFailedErrorPassthroughRule(c, account.Platform, dataBytes, failedMessage); matched {
						// 命中透传规则也要记录 ops 上游错误事件（对齐 CC/Messages 与
						// antigravity 先例），否则透传命中的 failed 在监控中不可见。
						s.recordOpenAIStreamUpstreamErrorWithUsage(c, account, true, upstreamRequestID, "http_error", dataBytes, failedMessage, usage)
						MarkResponseCommitted(c)
				...
				if openAIStreamClientOutputStarted(c, clientOutputStarted) && !postOutputFailureRecorded {
					s.recordOpenAIStreamUpstreamErrorWithUsage(
						c, account, true, upstreamRequestID, "http_error", dataBytes, failedMessage, usage,
						openAIStreamCommitState{
							ClientOutputStarted:        true,
							FirstClientOutputEventType: firstClientOutputEventType,
							VisibleOutputSeen:          visibleOutputSeen,
						},
					)
					postOutputFailureRecorded = true
				}
			...
			if lineStartsClientOutput && trimmedData != "[DONE]" && !openAIStreamEventTypeIsTerminal(eventType) {
				...
				if firstClientOutputEventType == "" {
					firstClientOutputEventType = strings.TrimSpace(eventType)
					if firstClientOutputEventType == "" {
						firstClientOutputEventType = "unknown"
					}
				}
			}
			...
			if lineStartsVisibleOutput {
				visibleOutputSeen = true
			}
			if firstTokenMs == nil && lineStartsVisibleOutput {
		...
		if !clientDisconnected {
			if !clientOutputStarted && !lineStartsClientOutput {
				...
				if line == "" {
					streamProgressObserved = streamProgressObserved || eventShowsProgress
					eventShowsProgress = false
				}
				continue
			}
	...
	if err := documentScanner.Err(); err != nil {
		...
		if !streamProgressObserved && !openAIStreamClientOutputStarted(c, clientOutputStarted) {
