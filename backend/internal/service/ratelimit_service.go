...
const (
	defaultRateLimit429CooldownSeconds = 300
...
func (s *RateLimitService) handlePoolModeRetryExhausted429(ctx context.Context, account *Account, headers http.Header, responseBody []byte) {
	if account == nil {
		return
	}
	// Reuse the complete reset-time parser without letting the normal pool-mode
	// guard skip the state write. The copy prevents this terminal path from
	// changing the account's pool-mode configuration in memory.
	accountCopy := *account
	if account.Credentials != nil {
		...
		delete(accountCopy.Credentials, "pool_mode")
	}
	s.handle429(ctx, &accountCopy, headers, responseBody)
}
