...
func TestOpenAI429FastPath_KeepsOAuthAccountSchedulableDuringExplicitShortRetryWindow(t *testing.T) {
	...
	apiKeyShouldDisable := svc.handleOpenAIAccountUpstreamError(context.Background(), apiKeyAccount, http.StatusTooManyRequests, headers, nil)
	...
	require.True(t, svc.shouldRetryOpenAIOAuth429OnSameAccountWithResponse(account, http.StatusTooManyRequests, false, headers, nil))
	require.True(t, svc.shouldRetryOpenAIOAuth429OnSameAccountWithResponse(setupTokenAccount, http.StatusTooManyRequests, false, headers, nil))
...
func TestOpenAI429FastPath_NoResetDoesNotRetrySameOAuthAccount(t *testing.T) {
...
func TestOpenAI429FastPath_LongRetryAfterDoesNotRetrySameOAuthAccount(t *testing.T) {
	svc := &OpenAIGatewayService{}
	...
	require.True(t, svc.isOpenAIAccountRuntimeBlocked(account))
	require.False(t, svc.shouldRetryOpenAIOAuth429OnSameAccountWithResponse(
		account,
