...
func (s *OpenAIGatewayService) proxyResponsesWebSocketV2Passthrough(
	...
	firstClientMessage = sanitizedFirstMessage
	if key := strings.TrimSpace(gjson.GetBytes(firstClientMessage, "prompt_cache_key").String()); key != "" {
		promptCacheKeysByModel[strings.TrimSpace(gjson.GetBytes(firstClientMessage, "model").String())] = key
	}
		...
		filter: func(msgType coderws.MessageType, payload []byte) (out []byte, blocked *OpenAIFastBlockedError, filterErr error) {
			...
			if policyErr == nil && blocked == nil {
				...
				if account.Platform == PlatformOpenAI && !isOpenAIResponsesCompactPath(c) && !frameMachineStaged {
					...
					if key := strings.TrimSpace(gjson.GetBytes(sanitized, "prompt_cache_key").String()); key != "" {
						promptCacheKeysByModel[mappedModel] = key
					}
				}
				out = sanitized
			}
