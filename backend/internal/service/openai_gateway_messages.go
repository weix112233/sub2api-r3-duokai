...
func (s *OpenAIGatewayService) ForwardAsAnthropic(
	...
	startTime := time.Now()
	// This path converts Anthropic Messages into an internal Responses-shaped
	// body, but it is still a compatibility bridge. Keep that distinction
	// explicit so native Responses-only outbound fields are not added later.
