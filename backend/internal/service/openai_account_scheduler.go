...
const (
	...
	// A single slow first token is not enough evidence to break a sticky
	// session. Requiring a short sample history and a cooldown prevents long
	// reasoning requests from causing an account-switch storm.
	openAIStickyEscapeMinTTFTSamples = 3
	openAIStickyEscapeCooldown       = 30 * time.Second
	openAIStickyEscapeDefaultTTFTMs  = 60000
)
...
type openAIAccountRuntimeStat struct {
	...
	lastStickyEscapeUnix atomic.Int64
}
...
func (s *openAIAccountRuntimeStats) ttftSamples(accountID int64) int64 {
	if s == nil || accountID <= 0 {
		return 0
	}
	value, ok := s.accounts.Load(accountID)
...
func (s *openAIAccountRuntimeStats) claimStickyEscape(accountID int64, cooldown time.Duration) bool {
	if s == nil || accountID <= 0 {
		return false
	}
	value, ok := s.accounts.Load(accountID)
	...
	cooldownNanos := cooldown.Nanoseconds()
	for {
		last := stat.lastStickyEscapeUnix.Load()
		if last > 0 && now-last < cooldownNanos {
			return false
		}
		if stat.lastStickyEscapeUnix.CompareAndSwap(last, now) {
			return true
		}
	}
}
...
func (s *defaultOpenAIAccountScheduler) Select(
	...
	// A response chain already has an authoritative account binding. Keep that
	// binding even when the input is independently rebuildable: moving a busy
	// chain to another account causes previous_response_id to be discarded or
	// rejected, forcing a full replay and multiplying input-token cost.
...
func (s *defaultOpenAIAccountScheduler) shouldEscapeStickyAccount(accountID int64, cfg openAIStickyEscapeConfig) (reason string, errorRate float64, ttft float64, shouldEscape bool) {
	...
	if hasTTFT &&
		s.stats.ttftSamples(accountID) >= openAIStickyEscapeMinTTFTSamples &&
		ttft > cfg.ttftMs &&
		s.stats.claimStickyEscape(accountID, openAIStickyEscapeCooldown) {
...
const maxOpenAIAccountPoolFallbackHops = 16

func isOpenAIAccountPoolExhausted(err error) bool {
	return errors.Is(err, ErrNoAvailableAccounts) ||
		errors.Is(err, ErrNoAvailableCompactAccounts)
}

func (s *OpenAIGatewayService) nextOpenAIAccountPoolFallbackGroupID(
	ctx context.Context,
	groupID *int64,
	platform string,
) (*int64, error) {
	if s == nil || s.schedulerSnapshot == nil || groupID == nil || platform != PlatformOpenAI {
		return nil, nil
	}

	group, err := s.schedulerSnapshot.GetGroupByIDLite(ctx, *groupID)
	if err != nil {
		return nil, fmt.Errorf("load OpenAI account-pool group %d: %w", *groupID, err)
	}
	if group == nil || group.Status != StatusActive || group.Platform != PlatformOpenAI ||
		group.FallbackGroupID == nil {
		return nil, nil
	}

	nextGroup, err := s.schedulerSnapshot.GetGroupByIDLite(ctx, *group.FallbackGroupID)
	if err != nil {
		return nil, fmt.Errorf("load OpenAI fallback group %d: %w", *group.FallbackGroupID, err)
	}
	if nextGroup == nil || nextGroup.Status != StatusActive || nextGroup.Platform != PlatformOpenAI {
		return nil, nil
	}
...
func (s *OpenAIGatewayService) selectAccountWithScheduler(
	...
	for hop := 0; ; hop++ {
		...
		if err == nil || !isOpenAIAccountPoolExhausted(err) ||
			hop >= maxOpenAIAccountPoolFallbackHops {
			return selection, decision, err
		}

		nextGroupID, fallbackErr := s.nextOpenAIAccountPoolFallbackGroupID(
			ctx,
			currentGroupID,
			platform,
		)
		...
		if _, seen := visited[*nextGroupID]; seen {
			return nil, decision, fmt.Errorf(
				"OpenAI account-pool fallback group cycle detected at group %d",
				*nextGroupID,
			)
		}
		visited[*nextGroupID] = struct{}{}
		slog.Info(
			"openai.account_pool_fallback",
			"from_group_id", derefGroupID(currentGroupID),
...
func (s *OpenAIGatewayService) openAIStickyEscapeConfig() openAIStickyEscapeConfig {
	if s != nil && s.cfg != nil {
		...
		if ttftMs <= 0 {
			ttftMs = openAIStickyEscapeDefaultTTFTMs
		}
