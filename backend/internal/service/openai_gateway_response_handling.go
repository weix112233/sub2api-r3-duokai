...
func (s *OpenAIGatewayService) handleStreamingResponseWithReasoning(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, startTime time.Time, originalModel, mappedModel, reasoningEffort string) (*openaiStreamingResult, error) {
	...
	completeGuardedEvent := func(queueDrained bool) {
		...
		completedEventShowsProgress := eventShowsProgress
		shouldFlush := eventShouldFlush || (queueDrained && clientOutputStarted)
		eventInProgress = false
		if completedEventShowsProgress {
			streamProgressObserved = true
	...
	finalizeStream := func() (*openaiStreamingResult, error) {
		...
		if !sawTerminalEvent && !streamProgressObserved && !openAIStreamClientOutputStarted(c, clientOutputStarted) && !eventShouldFlush {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			if eventType == "response.failed" {
				...
				if !openAIStreamClientOutputStarted(c, clientOutputStarted) {
					...
					if isOpenAIUpstreamCapacityShedEvent(dataBytes) {
						sawFailedEvent = true
						streamEarlyErr = s.newOpenAIStreamFailoverErrorWithUsage(c, account, false, upstreamRequestID, dataBytes, failedMessage, usage, resp.Header)
						return
					}
					if status, errType, errMsg, matched := applyOpenAIStreamFailedErrorPassthroughRule(c, account.Platform, dataBytes, failedMessage); matched {
						...
						s.recordOpenAIStreamUpstreamErrorWithUsage(c, account, false, upstreamRequestID, "http_error", dataBytes, failedMessage, usage)
				...
				if openAIStreamClientOutputStarted(c, clientOutputStarted) && !postOutputFailureRecorded {
					s.recordOpenAIStreamUpstreamErrorWithUsage(
						c, account, false, upstreamRequestID, "http_error", dataBytes, failedMessage, usage,
			...
			if startsClientOutput && !openAIStreamEventTypeIsTerminal(eventType) && firstClientOutputEventType == "" {
			...
			if guardFirstOutput {
				eventShowsProgress = eventShowsProgress || openAIStreamDataShowsProgress(data, eventType)
...
func (s *OpenAIGatewayService) bindHTTPResponseAccount(ctx context.Context, c *gin.Context, account *Account, responseID string) {
	...
	s.bindOpenAIResponsePromptCacheKey(ctx, c, account, responseID, openAIOutboundPromptCacheKeyFromContext(c))
}

func (s *OpenAIGatewayService) bindOpenAIResponsePromptCacheKey(
	_ context.Context,
	c *gin.Context,
	account *Account,
	responseID string,
	...
	if s == nil || account == nil || account.ID <= 0 || responseID == "" {
		return
	}
	key := promptCacheKey
	if key == "" {
		key = openAIOutboundPromptCacheKeyFromContext(c)
	}
	if key == "" {
		return
	}
	s.getOpenAIWSStateStore().BindResponsePromptCacheKey(
...
func sanitizeOpenAIResponseFailedEventForClient(payload []byte, eventType string, clientOutputStarted bool) ([]byte, bool) {
	...
	updated := payload
	// 首个语义输出前的容量降载由网关执行有界同账号重试；只有尚未提交输出的
	// 终态错误才允许保留客户端重试语义。客户端已经收到语义输出后，改写为
	// server_error 会触发 Codex 自动重放整段长上下文，放大已计费输入和连接风暴。
	// 此时必须保留真实 server_is_overloaded / slow_down，让当前部分响应终止，
	// 禁止客户端把同一 turn 自动完整重放。
