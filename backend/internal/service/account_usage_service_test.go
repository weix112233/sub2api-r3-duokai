...
func TestExtractOpenAICodexProbeUpdatesRejectsSuccessWithoutQuotaHeaders(t *testing.T) {
	t.Parallel()
	...
	if err == nil {
		t.Fatal("expected a successful response without quota windows to fail closed")
	}
	if updates != nil {
		t.Fatalf("expected no updates from an unauthoritative response, got %v", updates)
	}
}

func TestExtractOpenAICodexProbeUpdatesMarksProviderSuccessWhenQuotaAvailable(t *testing.T) {
	t.Parallel()
	...
	if !providerProbeSucceeded(updates) {
		t.Fatalf("expected provider availability marker, got %v", updates)
	}
}

func TestExtractOpenAICodexProbeUpdatesDoesNotMarkExhaustedQuotaAsSuccess(t *testing.T) {
	t.Parallel()
	...
	if providerProbeSucceeded(updates) {
		t.Fatalf("exhausted quota must not produce provider availability evidence: %v", updates)
	}
}

func TestExtractOpenAICodexProbeUpdatesRejectsIncompleteMalformedOrContradictoryMetrics(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			headers.Set("x-codex-primary-window-minutes", "300")
			headers.Set("x-codex-secondary-used-percent", "18")
			headers.Set("x-codex-secondary-reset-after-seconds", "604800")
			headers.Set("x-codex-secondary-window-minutes", "10080")
			tt.mutate(headers)
			...
			if providerProbeSucceeded(updates) {
				t.Fatalf("invalid metrics must fail closed, got %v", updates)
			}
		})
	}
}

func TestExtractOpenAICodexProbeUpdatesNeverAuthorizesServerFailure(t *testing.T) {
	t.Parallel()
	...
	for _, statusCode := range []int{http.StatusBadGateway, http.StatusServiceUnavailable} {
		...
		if err != nil {
			t.Fatalf("status %d extraction error = %v", statusCode, err)
		}
		if providerProbeSucceeded(updates) {
			t.Fatalf("status %d must not authorize recovery", statusCode)
		}
	}
}

func TestExtractOpenAICodexProbeUpdatesRejectsNonSuccessWithoutHeaders(t *testing.T) {
	t.Parallel()
	...
	if err == nil {
		t.Fatal("expected non-success response without headers to fail closed")
	}
	if updates != nil {
		t.Fatalf("expected no updates on failed probe, got %v", updates)
	}
}
