...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_KeepLeaseAcrossTurns(t *testing.T) {
	...
	writeMessage(`{"type":"response.create","model":"gpt-5.1","stream":false,"prompt_cache_key":"client-ingress-cache","input":[{"type":"input_text","text":"hello"}]}`)
	...
	writeMessage(`{"type":"response.create","model":"gpt-5.1","stream":false,"previous_response_id":"resp_ingress_turn_1","input":[{"type":"input_text","text":"follow up"}]}`)
	...
	firstPromptCacheKey := gjson.Get(requestToJSONString(captureConn.writes[0]), "prompt_cache_key").String()
	secondPromptCacheKey := gjson.Get(requestToJSONString(captureConn.writes[1]), "prompt_cache_key").String()
	require.Regexp(t, `^pcv2-[A-Za-z0-9_-]{43}$`, firstPromptCacheKey)
	require.LessOrEqual(t, len(firstPromptCacheKey), openAIOutboundPromptCacheKeyMaxLength)
	require.Equal(t, firstPromptCacheKey, secondPromptCacheKey, "后续帧省略 prompt_cache_key 时应复用首帧派生 key")
	require.NotContains(t, requestToJSONString(captureConn.writes[0]), "client-ingress-cache")
	require.NotContains(t, requestToJSONString(captureConn.writes[1]), "client-ingress-cache")
}
...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_PassthroughHeadersUsePromptCacheAndTurnState(t *testing.T) {
	...
	forwardedFirst := requestToJSONString(upstreamConn.writes[0])
	forwardedPromptCacheKey := gjson.Get(forwardedFirst, "prompt_cache_key").String()
	require.NotEmpty(t, forwardedPromptCacheKey)
	require.Equal(t, isolateOpenAISessionID(0, forwardedPromptCacheKey), captureDialer.lastHeaders.Get("session_id"))
	require.Empty(t, captureDialer.lastHeaders.Get(openAIWSTurnStateHeader))
	require.Empty(t, captureDialer.lastHeaders.Get(openAIWSTurnMetadataHeader))
