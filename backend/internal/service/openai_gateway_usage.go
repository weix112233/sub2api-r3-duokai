...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	usageFields := []zap.Field{
		...
		zap.Int("cache_read_tokens", usageLog.CacheReadTokens),
		zap.Int("cache_creation_tokens", usageLog.CacheCreationTokens),
		zap.Int("output_tokens", usageLog.OutputTokens),
		zap.Int("total_tokens", usageLog.TotalTokens()),
	...
	if result.FirstTokenMs != nil {
		usageFields = append(usageFields, zap.Int("first_token_ms", *result.FirstTokenMs))
	}
	if result.ServiceTier != nil {
		usageFields = append(usageFields, zap.String("service_tier", strings.TrimSpace(*result.ServiceTier)))
	}
	logger.L().Info("openai.usage_recorded", usageFields...)
