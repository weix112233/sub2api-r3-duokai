...
func openAIWSSemantic429Headers(account *Account, model string, headers http.Header) http.Header {
	...
	// Embedded stream/WS 429 events arrive under a successful HTTP handshake.
	// Do not reinterpret the handshake's quota snapshot as the event's account
	// reset. Preserve only an explicit short-release signal used by the bounded
	// same-account retry policy.
	retryAfter := headers.Values("Retry-After")
	if len(retryAfter) == 0 {
		return nil
	}
	filtered := make(http.Header, 1)
	filtered["Retry-After"] = append([]string(nil), retryAfter...)
	return filtered
}
