...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	if promptCacheKey == "" && !isOpenAIResponsesCompactPath(c) {
		previousResponseID := strings.TrimSpace(gjson.GetBytes(body, "previous_response_id").String())
		if previousResponseID != "" {
			groupID := getOpenAIGroupIDFromContext(c)
			if cachedKey, ok := s.getOpenAIWSStateStore().GetResponsePromptCacheKey(groupID, account.ID, previousResponseID); ok {
				body, err = sjson.SetBytes(append([]byte(nil), body...), "prompt_cache_key", cachedKey)
				if err != nil {
					return nil, fmt.Errorf("restore prompt cache key from previous response: %w", err)
				}
				originalBody = body
				requestView = newOpenAIRequestView(body)
				reqModel, reqStream, promptCacheKey = requestView.Model, requestView.Stream, requestView.PromptCacheKey
	...
	wsRetryLoop:
		...
		if wsErr == nil {
			...
			s.bindOpenAIResponsePromptCacheKey(
				ctx,
				c,
				account,
				wsResult.RequestID,
				openAIOutboundPromptCacheKeyFromContext(c),
			)
...
func (s *OpenAIGatewayService) buildUpstreamRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token string, isStream bool, promptCacheKey string, isCodexCLI bool) (*http.Request, error) {
	...
	clientPromptCacheKey := strings.TrimSpace(promptCacheKey)
	...
	// Capture the compatibility boundary before protocol-envelope sanitization.
	// The sanitizer removes prompt_cache_key by design, so re-detecting from
	...
	if account.Type == AccountTypeOAuth &&
		...
		// Messages compatibility requests are not Codex protocol requests.
		// Keep this removal after all identity/header builders and overrides.
		req.Header.Del("OpenAI-Beta")
		req.Header.Del("originator")
	}
