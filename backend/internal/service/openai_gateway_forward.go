...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	if wsDecision.Transport != OpenAIUpstreamTransportResponsesWebsocketV2 &&
		...
		previousResponseIDValue.Exists() {
		// ChatGPT OAuth anchors are credential-bound. Never forward a client
		// anchor through a scheduler-selected account because failover can cross
		// account namespaces and strand orphaned tool outputs.
		markPatchDelete("previous_response_id")
	} else if previousResponseIDValue.Exists() && strings.TrimSpace(previousResponseIDValue.String()) == "" {
		markPatchDelete("previous_response_id")
	}
