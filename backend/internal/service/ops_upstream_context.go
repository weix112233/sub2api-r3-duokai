...
const (
	...
	// OpenAIRequestScopedTransientKey 标记本次请求遇到与账号无关的上游
	// 容量降载。即使流已经向客户端写出、无法再构造 failover error，
	// handler 也不能把该请求计入账号健康失败率。
	OpenAIRequestScopedTransientKey = "openai_request_scoped_transient"
...
func MarkOpenAIRequestScopedTransient(c *gin.Context) {
	if c != nil {
		c.Set(OpenAIRequestScopedTransientKey, true)
	}
}

func IsOpenAIRequestScopedTransient(c *gin.Context) bool {
	if c == nil {
		return false
	}
	return c.GetBool(OpenAIRequestScopedTransientKey)
}
...
type OpsUpstreamErrorEvent struct {
	...
	// Stream commit metadata is intentionally content-free. It lets Ops
	// distinguish retry-safe pre-output failures from post-output failures
	// without storing model output or request content.
	ClientOutputStarted        bool   `json:"client_output_started,omitempty"`
	FirstClientOutputEventType string `json:"first_client_output_event_type,omitempty"`
	VisibleOutputSeen          bool   `json:"visible_output_seen,omitempty"`

	// Usage is the best-effort raw usage carried by a streamed response.failed
	// event. It is kept under the attempt event so failed provider consumption
	// remains auditable without entering the successful usage ledger.
