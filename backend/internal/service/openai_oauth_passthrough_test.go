...
func TestOpenAIGatewayService_OAuthMessagesBridgeDoesNotInjectDefaultInstructions(t *testing.T) {
	...
	require.Equal(t, isolateOpenAISessionID(0, "anthropic-metadata-session-1"), upstream.lastReq.Header.Get("Session_Id"))
...
func TestOpenAIGatewayService_OAuthPassthrough_CompactUsesJSONAndKeepsNonStreaming(t *testing.T) {
	...
	require.Regexp(t, `^[0-9a-f]{16}$`, upstream.lastReq.Header.Get("Session_Id"))
	require.Equal(t, "chatgpt.com", upstream.lastReq.Host)
	require.Equal(t, "chatgpt-acc", upstream.lastReq.Header.Get("chatgpt-account-id"))
...
func TestOpenAIGatewayService_APIKeyPassthrough_Transient5xxTriggersFailover(t *testing.T) {
	...
	for _, statusCode := range []int{
		...
		t.Run(fmt.Sprintf("status_%d", statusCode), func(t *testing.T) {
			...
			require.Contains(t, gjson.GetBytes(upstream.lastBody, "prompt_cache_key").String(), openAIOutboundPromptCacheKeyPrefix)
