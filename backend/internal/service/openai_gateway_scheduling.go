...
func shouldAutoPauseOpenAIAccountByQuota(ctx context.Context, account *Account) (bool, openAIQuotaAutoPauseDecision) {
	...
	// A provider-observed 7-day window at 100% is a hard scheduling boundary
	// until that same window resets. This is deliberately independent of the
	// configurable auto-pause threshold and per-account opt-out flags: allowing
	// an exhausted OAuth account back into the candidate set causes repeated
	// upstream failures and can incorrectly drive the dual-route fallback.
	if openAICodex7dQuotaResetActive(account.Extra, time.Now()) {
...
// observed exhausted 7-day window. Missing or malformed reset data fails open
// here; a bare 100% snapshot is not enough to infer that the current window is
// still exhausted after a provider rollover.
func openAICodex7dQuotaResetActive(extra map[string]any, now time.Time) bool {
	if readOpenAIQuotaUsedPercent(extra, "7d") < 100 {
		return false
	}
	resetAtRaw, ok := extra["codex_7d_reset_at"]
	if !ok || resetAtRaw == nil {
		return false
	}
...
func (s *OpenAIGatewayService) SelectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}) (*AccountSelectionResult, error) {
	...
	for hop := 0; ; hop++ {
		// 分组利润控制按当前池装门，fallback 后必须使用下一级分组自己的定价约束。
		currentCtx := s.withOpenAIProfitControlGate(ctx, currentGroupID)
