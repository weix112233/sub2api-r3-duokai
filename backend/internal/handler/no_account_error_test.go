...
func TestMarkSub2APINoAvailableAccounts_OnlyMarksConfirmed503(t *testing.T) {
	c := newTestGinContextWithRequest()
	markSub2APINoAvailableAccounts(c, noAccountErrorClassification{
		Status:  http.StatusServiceUnavailable,
		ErrType: "api_error",
	})
	require.Equal(t, "1", c.Writer.Header().Get(sub2APINoAvailableAccountsHeader))

	modelNotFound := newTestGinContextWithRequest()
	markSub2APINoAvailableAccounts(modelNotFound, noAccountErrorClassification{
		Status:        http.StatusNotFound,
		ErrType:       "model_not_found",
		ModelNotFound: true,
	})
	require.Empty(t, modelNotFound.Writer.Header().Get(sub2APINoAvailableAccountsHeader))
}

func TestOpenAINoAvailableAccountsAfterFailover_EmitsStandbySignal(t *testing.T) {
	...
	handled := h.handleOpenAINoAvailableAccountsAfterFailover(
		c,
		apiKey,
		"gpt-5.6-sol",
...
func TestOpenAINoAvailableAccountsAfterFailover_DoesNotConvertOtherErrors(t *testing.T) {
	c := newTestGinContextWithRequest()
	h := &OpenAIGatewayHandler{}
	apiKey := &service.APIKey{GroupID: ptrInt64(7)}
	...
	handled := h.handleOpenAINoAvailableAccountsAfterFailover(
		...
		"gpt-5.6-sol",
		service.PlatformOpenAI,
		fmt.Errorf("upstream rate limit"),
		false,
	)

	require.False(t, handled)
	require.Empty(t, c.Writer.Header().Get(sub2APINoAvailableAccountsHeader))
	require.Equal(t, 200, c.Writer.Status(), "no response should be committed for unrelated errors")
}
