...
func (s *httpUpstreamService) Do(req *http.Request, proxyURL string, accountID int64, accountConcurrency int) (*http.Response, error) {
	...
	if profile == service.HTTPUpstreamProfileOpenAI {
		// Final transport fallback for OpenAI HTTP callers that do not pass
		// through the gateway-specific request builders.
		service.SanitizeCodexOutboundRequest(req)
	}
...
func (s *httpUpstreamService) DoWithTLS(req *http.Request, proxyURL string, accountID int64, accountConcurrency int, profile *tlsfingerprint.Profile) (*http.Response, error) {
	...
	if upstreamProfile == service.HTTPUpstreamProfileOpenAI {
		// Keep the transport boundary fail-closed after all request builders
		// and account overrides have completed.
		service.SanitizeCodexOutboundRequest(req)
	}
