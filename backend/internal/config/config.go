...
func load(allowMissingJWTSecret bool) (*Config, error) {
	...
	if cfg.Gateway.OpenAIScheduler.StickyEscapeTTFTMs == 0 {
		// Normal long-context/reasoning requests can exceed 15s before the
		// first token. Keep TTFT escape opt-in at a conservative 60s default so
		// latency variance does not fragment sticky sessions.
		cfg.Gateway.OpenAIScheduler.StickyEscapeTTFTMs = 60000
	}
