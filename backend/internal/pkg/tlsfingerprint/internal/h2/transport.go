...
import (
	...
	"github.com/Wei-Shaw/sub2api/internal/pkg/tlsfingerprint/internal/h2/httpcommon"
...
func (t *Transport) maxHeaderListSize() uint32 {
	if t.HeaderListSize != nil {
		return *t.HeaderListSize
	}
...
type ClientConn struct {
	...
	lastPushID       uint32                   // highest server-promised stream, including cancelled pushes
...
func (t *Transport) newClientConn(c net.Conn, singleUse bool, internalStateHook func()) (*ClientConn, error) {
	if t.InitialWindowSize != nil && *t.InitialWindowSize > math.MaxInt32 ||
		t.ConnectionWindowUpdate != nil && *t.ConnectionWindowUpdate > math.MaxInt32-initialWindowSize {
		c.Close()
		return nil, errors.New("http2: profile receive window exceeds protocol limit")
	}
	...
	cc.fr.exactHeaderListSize = t.HeaderListSize
	...
	push := uint32(0)
	if t.EnablePush != nil && *t.EnablePush {
		push = 1
	}
	...
	if max := t.maxHeaderListSize(); max != 0 || t.HeaderListSize != nil {
	...
	if conf.MaxUploadBufferPerConnection > 0 {
...
func (rl *clientConnReadLoop) processData(f *DataFrame) error {
	...
	if cs == nil {
		...
		cancelledPush := f.StreamID%2 == 0 && f.StreamID <= cc.lastPushID
		cc.mu.Unlock()
		if f.StreamID >= neverSent && !cancelledPush {
...
func (rl *clientConnReadLoop) processPushPromise(f *PushPromiseFrame) error {
	cc := rl.cc
	if cc.t.EnablePush == nil || !*cc.t.EnablePush {
		return ConnectionError(ErrCodeProtocol)
	}
	// This gateway has no push consumer or push cache. Accept the promise,
	// advance HPACK state (including continuations), then cancel that stream.
	// Cancelling a promised stream is explicitly allowed by RFC 9113 8.4.
	streamID, promiseID := f.StreamID, f.PromiseID
	cc.mu.Lock()
	valid := streamID%2 == 1 && streamID < cc.nextStreamID &&
		promiseID != 0 && promiseID%2 == 0 && promiseID > cc.lastPushID
	if valid {
		cc.lastPushID = promiseID
	}
	cc.mu.Unlock()
	if !valid {
		return ConnectionError(ErrCodeProtocol)
	}
	decoder := cc.fr.ReadMetaHeaders
	decoder.SetEmitEnabled(false)
	defer decoder.SetEmitEnabled(true)
	fragment, ended := f.HeaderBlockFragment(), f.HeadersEnded()
	var size uint64
	for {
		size += uint64(len(fragment))
		// Bound compressed work even when the peer withholds END_HEADERS.
		if size > uint64(cc.fr.maxHeaderListSize())+4096 {
		...
		fragment, ended = continuation.HeaderBlockFragment(), continuation.HeadersEnded()
	}
	...
	cc.wmu.Lock()
	defer cc.wmu.Unlock()
	if err := cc.fr.WriteRSTStream(promiseID, ErrCodeCancel); err != nil {
		return err
	}
	return cc.bw.Flush()
}
